...
We now look at how matrix of numbers is converted to actual image. Let"s think about 
...
Try change the LUT by \ijmenu{[Image > Lookup Tables >Spectrum]}. 
Pixel value does not change by this operation, but the pixel intensity changes 
that the image appears differently now. Check the LUT again by do \ijmenu{[Image > Color > Show LUT]}. 
...
you should not compress data, if you want to get some numbers out of them.
...
Multidimensional data, in which we define here as a set of image data with dimensions more than x and y, or 2D, have intrinsic limitation in displaying them on two dimensional screen. Efforts have been made to represent multidimensionality in various ways. One way is the ``image stack''.

\subsubsection{stacks}
When you take time-lapse sequence or z-sections using commercial microscope system, image files are generally saved in the company specific file formats (\textit{e.g.} .lif files). Importing these images into ImageJ could simply be done using LOCI bioformats plugin. The image is then appears as a series of images contained within a window with a scroll bar at the bottom. By scrolling, one could go through the third dimension like a movie. This is the simplest form of multidimensional representation in 2D. 
...
By importing such files, we can create a stack in ImageJ. In Metamorph, one of the software used in biology, uses .stk as the file extension. These stk files can also be loaded directly in ImageJ. 
...
\ijmenu{[Start Animation]} plays the sequence as animation. Within the main manue tree, this command resides at \ijmenu{[Image > Stacks > Tools > Start Animation]}. 
...
There are many occasions that you need to edit stacks such as
\begin{itemize}
\item Truncate a stack because you only need to analyze part of the movie.
\item To make a presentation you want to combine two different stacks.
\item Need to add a stack at the end of another stack.
\item To make a stack by attaching each frame in a stack by a frame of another, such as to have two channels side-by side,
\item You need only every second frame or slices to reduce the stack size by a half. 
\item To split two channel time series to individual time points. 
\item You need to insert a small inset within a stack. 
\end{itemize}
For such demands, tools for stack resides below \ijmenu{[Image > Stack > Tools]}. Figure \ref{fig:editStack1} and figure \ref{fig:editStack2} schematically shows what various commands does. 
%figure
\begin{figure}[hbtp]
\begin{center}
\includegraphics[width=\textwidth]{fig/stackEditing1.png}
\caption{Editing Stacks 1. Courtesy of Sebasti\'{e}n Tosi, IRB Barcelona.}
...
\caption{Editing Stacks 2. Courtesy of Sebasti\'{e}n Tosi, IRB Barcelona.}
\label{fig:editStack2}
\end{center}
\end{figure}


\subsubsection{Visualization of Stacks}
...
The order we see common is the first one (XYZT) but in some
cases you might find out that the order is the second one (XYTZ) .
